...
public class AlertAnalysis {
	...
	public static String[] elapsedTimeFields = {"pxTotalReqTime","pxDeclarativeRulesInvokedElapsed",
		...
		"pxCommitElapsed","pxJavaCompileElapsed","pxTotalReqCPU","pxRuleIOElapsed","pxOtherBrowseElapsed","pxInferGeneratedJavaElapsed", "pxInferGeneratedJavaHLElapsed"};
	...
	{
		unfilteredAlerts = new ArrayList<AlertData>(theData);
	...
	public void initAlertAnalysis(List<AlertData> theData)
	{
		clearOutData();
		...
		for (AlertData a : theData)
		{
			String aUniqInteraction = a.getUniqueInteraction();
			
			alertsByID.put(""+aUniqInteraction, a);
	...
	public List<AlertGroup> getAlertGroups()
	{
		...
		for (String k : keys)
		{
			...
			g.setAlerts(res.get(k));
			g.setPalData(AlertAnalysis.summarizePAL(g.getAlerts()));
			g.setGroupLabel(k);
			groups.add(g);
		}
		alertGroups = groups;
	...
	public List<AlertGroup> getGroupedPega05(boolean removeConstants)
	{
		...
		for (AlertData a : pega05s)
		{
			...
			//if (eventKey.length()>150)
			//	eventKey = eventKey.substring(0,150);		//this messes stuff up
		...
		for (String k : keys)
		{
			cnt++;
			AlertGroup g = new AlertGroup();
			g.setAlertID("SQL"+cnt);
			g.setAlerts(res.get(k));
			g.setPalData(null);
			g.setGroupLabel(k);
			groups.add(g);
			pega05Groups.put(g.getAlertID(),g);
		}
	...
	public List<AlertGroup> getGroupedPega20()
	{
		...
		for (String k : keys)
		{
			...
			g.setGroupLabel(k);
			groups.add(g);
			pega20Groups.put(g.getAlertID(),g);
		}
	...
	public static HashMap<String,PALField> summarizePAL(AlertData alert)
	{
		...
		if (unAcct<0)
			unAcct = 0;
		HashMap<String, PALField> fields = new HashMap<String,PALField>();
		PALField uField = new PALField();
		uField.setCount(1);
		uField.setField("Unaccounted For");
		uField.setValue(unAcct);
		uField.setTotalTime(totalTime);
		fields.put("Unaccounted For", uField);
		Set<String> keys = res.keySet();
		for (String k : keys)
		{
			PALField field = new PALField();
	...
	public static String parse20Line(String l)
	{
		
		int endidx = l.indexOf("#");
		if  (endidx<0)
			endidx = l.length();
		return l.substring(0,endidx).trim();
	}
	
	public static String parse05Line(String l)
	{
		l = l.replaceAll("<", "&lt;");
		l = l.replaceAll(">", "&gt;");
		
		int startidx = l.indexOf("SQL:")+5;
		int endidx = l.indexOf("inserts:");
		if  (endidx<startidx)
			endidx = l.length();
		return l.substring(startidx,endidx).trim().replace("\'","\\\'");
	}
	...
	private static String removeSQLConstants(String sql)
	{
		...
		if (startCase>0)
		{
			...
			while (true){
				int sQuote = sql.indexOf('\'');
				if (sQuote<0 || sQuote > endCase)
					break;
				int eQuote = sql.indexOf('\'',sQuote+1);
				String tmp = sql.substring(sQuote+1,eQuote);
				sql = sql.replace("\'"+tmp+"\'", "???");
			}
		}
		...
		try{
			...
			while (true){
				int sQuote = where.indexOf('\'');
				if (sQuote<0)
					break;
				int eQuote = where.indexOf('\'',sQuote+1);
				String tmp = where.substring(sQuote+1,eQuote);
	...
	public static double getPalDouble(String v)
	{
		if (v == null)
			return 0;
		try
